...
(整个流程旨在将M&A事件中的`Acquiror`（收购方）和`Target`（目标方）公司名称与**专利数据库**及**Compustat数据库**进行自动化匹配与验证。系统通过名称清洗、模糊匹配(Fuzzy Match)以及基于大模型(Gemini API)的严格甄别，最终确保极高的匹配精度。)
...
(强烈建议用少量时间查看生成的手动审核黑箱文件。人工将判断不对的名字行删除后，再次运行 Step 2 或 Step 4 即可建立绝对无误的词典体系与终极合并报表。)
